...
export class Ball {
    ...
    static Init(): IsometricGroup {
        ...
        const circle = new IsometricCircle({ radius: 0.5, left: 0.5, right: 1.5, planeView: PlaneView.TOP } as IsometricCircleProps);
        ...
        for (let i = 1; i <= layers; i++) {
            // 计算当前层的高度
            const t = (i / layers) - 0.5;
            const top = t + 0.5;

            // 计算当前层圆的半径
            const layerRadius = Math.sqrt(ballRadius * ballRadius - t * t);
            console.log("top", top, "layerRadius", layerRadius);
